...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/02 17:33:38 by mvelazqu          #+#    #+#             */
/*   Updated: 2024/03/02 17:42:46 by mvelazqu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || fd >= OPEN_MAX)
		return (NULL);
	fail_info = 0;
	text = get_text(fd, keep[fd]);
	if (!text)
		return (keep[fd] = NULL, NULL);
	line = get_line(text);
	if (!line)
		return (keep[fd] = NULL, NULL);
	keep[fd] = get_keep(text, &fail_info);
	if (fail_info == FAIL)
		return (free(line), NULL);
	return (line);
}
